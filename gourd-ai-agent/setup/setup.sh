...
if [ -L "/usr/local/bin/gourdai" ]; then
    SYMLINK_EXISTS=true
fi
...
if [ "$SYMLINK_EXISTS" = true ]; then
    echo -e "You can now run: ${CYAN}gourdai cli${NC} or ${CYAN}gourdai web 0${NC}"
else
    echo -e "To use gourdai immediately, run:"
    echo -e "  ${CYAN}source ~/.${USER_SHELL}rc${NC}"
    echo ""
    echo -e "Then run: ${CYAN}gourdai cli${NC} or ${CYAN}gourdai web 0${NC}"
fi

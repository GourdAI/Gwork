...
public class GitService {
    ...
    /**
     * 每次请求的目标 Git 目录覆盖（Code 模式按所选项目重定向）。
     * <p>为避免改动 20+ 处 {@link #runGitCommand} 调用签名，用 ThreadLocal 在一次请求内
     * 覆盖执行目录；请求结束务必在 finally 里 {@link #clearDir()} 清理。</p>
     */
    ...
    /**
     * 在指定项目根目录下执行一段 Git 业务（Code 模式按所选项目重定向）。
     * <p>用 try/finally 确保 ThreadLocal 目录覆盖只在本次调用内生效。root 为空则用启动工作区。</p>
     *
     * @param root 项目根目录绝对路径（可空）
     * @param action 实际的 Git 业务调用（如 {@code this::status}）
     */
    public <T> T inRoot(String root, GitAction<T> action) throws Exception {
        useDir(root);
        try {
            return action.run();
        } finally {
    ...
    /**
     * 设置本次请求的 Git 目标目录（Code 模式传所选项目根；为空则用启动工作区）。
     */
    private void useDir(String root) {
        if (root != null && !root.trim().isEmpty() && root.indexOf("..") < 0) {
            currentDir.set(new File(root.trim()));
        } else {
            currentDir.remove();
        }
    }

    /** 清理 ThreadLocal，防止线程复用导致目录串台。 */
    private void clearDir() {
        currentDir.remove();
    }

    /** 当前生效的 Git 执行目录（请求覆盖优先，否则启动工作区）。 */
    private File effectiveDir() {
        File d = currentDir.get();
        return d != null ? d : workspaceDir;
    }
     ...
     *
     * @return 包含 gitAvailable、initialized、branch、changed、staged、untracked、files 的结果对象；
     *         files 为逐文件条目（path + status 分桶 + type 变更类型 A/M/D/U），供前端按类型渲染图标
    ...
    public Result<Map> status() throws Exception {
        ...
        if (checkRepo.exitCode != 0) {
            data.put("initialized", false);
            data.put("workspacePath", effectiveDir().getAbsolutePath());
            return Result.succeed(data);
        }
        ...
        for (String line : statusResult.stdout.split("\n")) {
            ...
            if ("?".equals(x) && "?".equals(y)) {
                ...
                files.add(fileEntry(filePath, "?", "U"));
            } else {
                if (!" ".equals(x) && "?".equals(x) == false) staged.add(filePath);
                if (!" ".equals(y) && "?".equals(y) == false) changed.add(filePath);

                // 变更类型（决定图标）：新增 A / 删除 D / 其余（含改名 R、类型变更 T）归为修改 M
                String type;
                if ("A".equals(x) || "A".equals(y)) type = "A";
                else if ("D".equals(x) || "D".equals(y)) type = "D";
                else type = "M";

                // 暂存分桶（决定操作按钮）：暂存区有变更 → S，否则仅工作区变更 → M
                String status = (!" ".equals(x) && "?".equals(x) == false) ? "S" : "M";
                files.add(fileEntry(filePath, status, type));
            }
        }
    ...
    /**
     * 构造逐文件状态条目（path + status 分桶 + type 变更类型）。
     */
    private static Map<String, String> fileEntry(String path, String status, String type) {
    ...
    public Result<Map> init(Boolean initialCommit) throws Exception {
        ...
        File gitignore = new File(effectiveDir(), ".gitignore");
        if (!gitignore.exists()) {
            String content = String.join("\n",
                    "# Auto-generated by GWork",
                    ".git/",
                    ".idea/",
                    ".gwork/",
                    ".gourdai/",
        ...
        if (Boolean.TRUE.equals(initialCommit)) {
            ...
            runGitCommand("git", "-c", "user.name=GWork",
                    "-c", "user.email=gourdai@noear.org",
    ...
    /**
     * 获取 Git Diff 内容。
     * <p>单文件查看时输出<b>完整文件</b>内容（而非默认仅 3 行上下文的局部片段），
     * 以便在审查面板中完整审查。策略：</p>
     * <ul>
     *   <li>已有提交（HEAD 存在）：用 {@code git diff HEAD} 一次性取相对最近提交的全部
     *       未提交变更（含已暂存 + 未暂存），避免两份 diff 拼接导致整份文件重复出现；</li>
     *   <li>尚无提交：合并未暂存与已暂存两份 diff；</li>
     *   <li>未跟踪新文件：git diff 不含未跟踪文件，直接读取工作区文件合成「全部新增」diff；</li>
     *   <li>单文件用超大上下文（{@code --unified})，超过 {@code MAX_DIFF_LINES} 行时截断保护。</li>
     * </ul>
     *
    ...
    public Result<Map> diff(String path) throws Exception {
        ...
        // 上下文行数：单文件审查时用超大值让 git 输出整份文件作为上下文，
        // 使审查面板展示完整文件内容（默认仅 3 行上下文，无法完整审查）。
        String contextArg = hasPath ? "--unified=100000" : "--unified=3";

        // 是否已有提交（HEAD 存在）。有则用 `git diff HEAD` 一次拿到相对最近提交的
        // 全部未提交变更（含已暂存 + 未暂存），避免把未暂存与已暂存两份 diff 拼接、
        // 同一文件整份内容重复出现两次。
        boolean headExists = runGitCommand("git", "rev-parse", "--verify", "HEAD").exitCode == 0;

        String fullDiff;
        if (headExists) {
            List<String> cmd = new ArrayList<>(Arrays.asList("git", "diff", "HEAD", contextArg));
            if (hasPath) { cmd.add("--"); cmd.add(path); }
            fullDiff = runGitCommand(cmd.toArray(new String[0])).stdout;
        } else {
            // 尚无任何提交：合并未暂存与已暂存两份 diff
            List<String> unstagedCmd = new ArrayList<>(Arrays.asList("git", "diff", contextArg));
            if (hasPath) { unstagedCmd.add("--"); unstagedCmd.add(path); }
            String unstaged = runGitCommand(unstagedCmd.toArray(new String[0])).stdout;

            List<String> stagedCmd = new ArrayList<>(Arrays.asList("git", "diff", "--cached", contextArg));
            if (hasPath) { stagedCmd.add("--"); stagedCmd.add(path); }
            String staged = runGitCommand(stagedCmd.toArray(new String[0])).stdout;

            fullDiff = unstaged;
            if (!staged.isEmpty()) {
                fullDiff += (fullDiff.isEmpty() ? "" : "\n") + staged;
            }
        }

        // 未跟踪的新文件：上述 diff 不含未跟踪文件；直接读取工作区文件，
        // 合成「全部新增」的 unified diff，保证新文件也能完整审查。
        if (hasPath && fullDiff.trim().isEmpty()) {
            String synthetic = synthesizeNewFileDiff(path);
            if (synthetic != null) {
                fullDiff = synthetic;
            }
        }

        // 截断保护：单文件完整 diff 限制 MAX_DIFF_LINES 行
        final int MAX_DIFF_LINES = 5000;
        if (hasPath) {
            String[] lines = fullDiff.split("\n");
            if (lines.length > MAX_DIFF_LINES) {
                fullDiff = String.join("\n", Arrays.copyOf(lines, MAX_DIFF_LINES))
                        + "\n\n... (文件过大，仅显示前 " + MAX_DIFF_LINES + " 行，请在终端查看完整内容)";
            }
        }
    ...
    /**
     * 为未跟踪的新文件合成一份「全部新增」的 unified diff。
     * <p>{@code git diff} 不包含未跟踪文件，因此审查面板对新文件拿不到内容。
     * 这里直接读取工作区文件，把每一行标为新增（{@code +}），拼出与 git 输出
     * 同构的 unified diff，让前端并排视图能完整展示新文件内容。</p>
     *
     * <p>二进制文件（含 NUL 字节）或读取失败时返回 {@code null}，由调用方回退。</p>
     *
     * @param path 相对仓库根的文件路径
     * @return 合成的 unified diff 文本；无法处理时返回 null
     */
    private String synthesizeNewFileDiff(String path) {
        try {
            File f = new File(effectiveDir(), path);
            if (!f.isFile()) return null;

            byte[] bytes = java.nio.file.Files.readAllBytes(f.toPath());
            // 简单二进制检测：含 NUL 字节视为二进制，不展示内容
            for (byte b : bytes) {
                if (b == 0) {
                    return "diff --git a/" + path + " b/" + path + "\n"
                            + "new file\nBinary file (未跟踪二进制文件，无法展示内容)\n";
                }
            }

            String content = new String(bytes, "UTF-8");
            String[] lines = content.split("\n", -1);
            // 末尾若有空串（源自结尾换行）去掉，避免多渲染一行空行

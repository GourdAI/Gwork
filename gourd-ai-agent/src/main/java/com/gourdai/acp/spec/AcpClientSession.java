...
package com.gourdai.acp.spec;

import com.gourdai.acp.json.TypeRef;
import com.gourdai.acp.error.AcpErrorCodes;
import com.gourdai.acp.error.AcpProtocolException;
import com.gourdai.acp.util.Assert;
		...
		switch (method) {
			case AcpSchema.METHOD_FS_READ_TEXT_FILE:
				return new MethodNotFoundError(method, "File system read not supported",
						Collections.singletonMap("reason", "Client does not have fs.readTextFile capability"));
			case AcpSchema.METHOD_FS_WRITE_TEXT_FILE:
				return new MethodNotFoundError(method, "File system write not supported",
						Collections.singletonMap("reason", "Client does not have fs.writeTextFile capability"));
			case AcpSchema.METHOD_SESSION_REQUEST_PERMISSION:
				return new MethodNotFoundError(method, "Permission request not supported",
						Collections.singletonMap("reason", "No requestPermissionHandler registered - use --yolo flag or register a handler"));
			...
			case AcpSchema.METHOD_TERMINAL_KILL:
				return new MethodNotFoundError(method, "Terminal not supported",
						Collections.singletonMap("reason", "Client does not have terminal capability"));
			default:

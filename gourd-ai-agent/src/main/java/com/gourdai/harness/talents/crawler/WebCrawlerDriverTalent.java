 ...
 */
@Preview("3.9.1")
public class WebCrawlerDriverTalent extends AbsTalent {
    private static final Logger LOG = LoggerFactory.getLogger(WebCrawlerDriverTalent.class);

    private final String apiKey;
    private final CrawlerDriver driver;

    public WebCrawlerDriverTalent(CrawlerDriver driver, String apiKey) {
        this.driver = driver;
        this.apiKey = apiKey;
    }

    @ToolMapping(name = "crawl_url", description = "读取并分析指定 URL 网页的详细内容")
    public String crawl_url(@Param("url") String url) {

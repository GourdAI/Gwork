...
public class AssistantMessage extends ChatMessageBase<AssistantMessage> {
    ...
    /**
     * 为尚无摘要的协议状态绑定语义摘要。
     * <p>4.1.1 起摘要口径<b>按协议分域</b>（见 {@link MessageSemanticHasher#hashFor}）：
     * 只存凭证的协议（OpenAI Responses / Gemini）用身份锚点摘要，不含 text / thinking，
     * 因而 {@code AgentUtil.dedupeAggregatedThinking} 之类对思考的归一化不会使其失效；
     * 存有逐字内容副本且副本必须与语义严格一致的协议（Anthropic Messages，其 signature
     * 与 thinking 文本逐字绑定）仍用全文摘要，保留保护性判废。</p>
     *
     * <p>已有摘要的状态一律不覆写：持久化数据载回时必须保持其原口径，
     * 否则旧会话的校验会因口径切换而全部失败。</p>
     */
    private void bindProtocolStateSemanticHashesInternal() {
        if (protocolStates != null) {
            for (Map.Entry<String, MessageProtocolState> entry : protocolStates.entrySet()) {
                MessageProtocolState state = entry.getValue();
                if (state == null) {
                    continue;
                }
                if (Utils.isEmpty(state.getSemanticHash())) {
                    state.setSemanticHash(MessageSemanticHasher.hashFor(entry.getKey(), this));
                }
                state.freeze();
            }
        }

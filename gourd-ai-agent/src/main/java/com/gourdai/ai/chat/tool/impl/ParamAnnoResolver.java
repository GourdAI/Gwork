package com.gourdai.ai.chat.tool.impl;

import org.noear.eggg.TypeEggg;
import org.noear.solon.Utils;
import com.gourdai.ai.util.ParamDesc;
...
public class ParamAnnoResolver implements BiFunction<AnnotatedElement, TypeEggg, ParamDesc> {
    ...
    public ParamDesc apply(AnnotatedElement element, TypeEggg typeEggg) {
        ...
        if (p1Anno != null) {
            String name = Utils.valueOr(p1Anno.value(), p1Anno.name());

            if (element instanceof Parameter) {
                Parameter p1 = (Parameter) element;
                name = Utils.valueOr(name, p1.getName());
                return new ParamDesc(name, typeEggg.getGenericType(), p1Anno.required(), p1Anno.description(), p1Anno.defaultValue(), p1Anno.format());
            } else {

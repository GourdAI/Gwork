...
public interface ChatMessage extends Serializable {
     ...
     * 从 json 反序列化为消息
     *
     * <p>历史消息的 blocks/toolCalls 里写的是迁移前的旧类名（org.noear.solon.ai.*），
     * 直接解析会抛 Unsupported type 导致整条消息读不出来；故先过一遍兼容层做定点改写。
     * 已迁移的数据只付一次 indexOf 短路开销。兼容层可整体摘除，见 {@link LegacyTypeCompat}。</p>
     *
     * <p>这里复用兼容层的共享只读 Options（内含 {@link Feature#Read_AutoType}）：
     * 本方法是热路径（每条历史消息调一次），而 ofJson 的 Feature 变参重载每次都会新建 Options。</p>
     */
    static ChatMessage fromJson(String json) {
        ONode oNode = ONode.ofJson(LegacyTypeCompat.rewrite(json), LegacyTypeCompat.readOptions());

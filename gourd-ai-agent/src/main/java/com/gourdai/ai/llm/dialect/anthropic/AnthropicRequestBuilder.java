...
public class AnthropicRequestBuilder {
    ...
    /**
     * 历史轮思考块被剥离后、content 会空掉时使用的占位正文。
     *
     * <p>不能用空串：Anthropic 既拒绝空 content 数组，也拒绝空 text 块。</p>
     *
     * @since 4.1.1
     */
    private static final String THINKING_ONLY_PLACEHOLDER_TEXT = "(thinking omitted)";
    ...
    public ONode build(ChatConfig config, ChatOptions options, List<ChatMessage> messages, boolean isStream) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            if (message instanceof ToolMessage) {
                ...
                ONode messageNode = buildMessageNode(message);
                if (message instanceof AssistantMessage
                        && !ThinkingReplayWindow.isWithinCurrentTurn(lastUserIdx, i, messages.size())) {
                    stripHistoricalThinkingBlocks(messageNode);
                }
        ...
        // 注意：thinking 终检与滚动缓存断点都推迟到 thinking 开关真正写入 root 之后执行，
        // 见下方 applyUnifiedThinkingOptions 之后的调用处（顺序理由在那里说明）。
        ...
        // thinking 启用时的末尾合法性终检。
        //
        // 【顺序是约束，不是风格】必须同时满足两个条件，归并后只有这一个位置：
        //  1) 必须在 applyUnifiedThinkingOptions / clampThinkingBudgetToMaxTokens 【之后】——
        //     thinking 开关是在那里才真正写进 root 的。早于它调用时 root.hasKey("thinking")
        //     恒为 false，本方法会立即 return —— 即【整个终检成为死代码】。
        //     （这正是 4.1.1 首版的真实缺陷：终检写在构建 messages 后紧接着，从未生效。
        //      由 AnthropicThinkingReplayWindowTest 用真实出站 JSON 捕获。）
        //  2) 必须在 applyMessageCacheBreakpoints 【之前】——终检可能改写末条 assistant
        //     的 content，断点若先挂会落在被改写的块上。
        enforceThinkingLeadingBlock(root, messagesNode);
    ...
    /**
     * thinking 启用时，保证末条 assistant 消息不会以非 thinking 块开头而被端点拒绝。
     *
     * <p><b>背景</b>：{@code AnthropicMessageStateSupport.resolveData} 在语义摘要失配时整包 fail-closed
     * （这是刻意的：signature 只对逐字相同的 thinking 有效，宁可不下发也不能下发无效签名）。
     * 但在「thinking 开启 + 末条 assistant 带 tool_use」的组合下，判废会令 thinking 块缺失，
     * content 以 {@code text} / {@code tool_use} 开头，而 Anthropic 要求此时必须以 thinking 块开头
     * —— 结果是 400 且不可自愈（消息已在历史里，每次重试精确复现）。</p>
     *
     * <p><b>修法</b>：不伪造签名（那会被验签拒绝，且违背 fail-closed 的初衷），
     * 而是去掉本轮请求的 {@code thinking} 开关：既然没有合法凭证可供延续，
     * 本轮以非思考模式完成优于整个会话卡死。</p>
     *
     * <p><b>不能只看末条消息</b>：协议原文是「final assistant message must start with a thinking
     * block (<i>preceding the lastmost set of tool_use and tool_result blocks</i>)」。括号里那半句
     * 正是 agent 主场景——{@code [user, assistant(tool_use), tool_result]} 里违规的 assistant 位于
     * {@code size-2}，末条是承载 tool_result 的 user。故必须从尾部向前跳过 tool_result 形态的 user
     * 消息，定位到真正的「最后一条 assistant」再判首块。
     * 真正的新 user 轮（含普通文本）则不受此约束，遇到即早退。</p>
     *
     * @since 4.1.1
     */
    private static void enforceThinkingLeadingBlock(ONode root, ONode messagesNode) {
        if (root == null || !root.hasKey("thinking") || messagesNode == null || !messagesNode.isArray()) {
            return;
        }
        if (messagesNode.size() == 0) {
            return;
        }
        if ("disabled".equals(root.get("thinking").get("type").getString())) {
            // 显式关闭思考：既无「必须以 thinking 块开头」的约束，也没有可撤的开关。
            // 不早退会把 type=disabled 误当成启用态撤掉，并打出误导性告警。
            return;
        }

        ONode last = null;
        for (int i = messagesNode.size() - 1; i >= 0; i--) {
            ONode node = messagesNode.get(i);
            if (node == null) {
                continue;
            }
            if ("assistant".equals(node.get("role").getString())) {
                last = node;
                break;
            }
            if (isToolResultOnlyMessage(node)) {
                // tool_result 承载块：属于「lastmost set of tool_use and tool_result」，继续向前找。
                continue;
            }
            // 真正的新 user 轮 —— 不受约束。
            return;
        ...
        ONode content = last.getOrNull("content");
        if (content == null || !content.isArray() || content.size() == 0) {
            return;
        }

        String firstType = content.get(0).get("type").getString();
        if ("thinking".equals(firstType) || "redacted_thinking".equals(firstType)) {
            return;
        }

        boolean hasToolUse = false;
        for (int i = 0; i < content.size(); i++) {
            if ("tool_use".equals(content.get(i).get("type").getString())) {
                hasToolUse = true;
                break;
            }
        }
        if (!hasToolUse) {
            // 无 tool_use 的末条 assistant 是 prefill 形态，不受「必须以 thinking 开头」约束。
            return;
        }

        root.remove("thinking");
        if (LOG.isWarnEnabled()) {
            LOG.warn("ai: anthropic thinking disabled for this turn -- "
                    + "last assistant message carries tool_use but has no valid thinking signature to replay");
        }
    }

    /**
     * 判断消息节点是否为「纯 tool_result 承载体」。
     *
     * <p>工具结果在出站时被合并成 {@code role=user} 的消息（见消息装配循环里的
     * {@code pendingToolResultNode}），与真正的新 user 轮在 JSON 里同为 user，
     * 唯一可靠的区分依据是 content 块全部为 {@code tool_result}。</p>
     *
     * @since 4.1.1
     */
    private static boolean isToolResultOnlyMessage(ONode messageNode) {
        if (messageNode == null || !"user".equals(messageNode.get("role").getString())) {
            return false;
        }
        ...
        for (int i = 0; i < content.size(); i++) {
            if (!"tool_result".equals(content.get(i).get("type").getString())) {
                return false;
            }
        }
        return true;
    ...
    /**
     * 剥离历史轮（最后一条 user 之前）assistant 消息里的 thinking / redacted_thinking 块。
     *
     * <p>跨 user 轮边界的思考 Anthropic 并不要求回传，而 {@code contentBlocksRaw} 逐字回放路径会
     * 把整个思考块原文带回去，逐轮累加即膨胀。本方法在节点层面统一处理，
     * 同时覆盖「逐字 raw 回放」与「语义字段重建」两条路径。</p>
     *
     * <p><b>保留当轮不动</b>：thinking 启用时 Anthropic 要求最后一条 assistant 消息
     * 必须以 thinking 块开头，而那正是当轮消息。</p>
     *
     * <p>剥后 content 数组为空时补一个空文本块：Anthropic 拒绝 content 为空数组的消息。</p>
     *
     * @since 4.1.1
     */
    private static void stripHistoricalThinkingBlocks(ONode messageNode) {
        if (messageNode == null) {
            return;
        }
        ONode content = messageNode.getOrNull("content");
        ...
        List<ONode> kept = new java.util.ArrayList<>();
        boolean removed = false;
        for (int i = 0; i < content.size(); i++) {
            ONode block = content.get(i);
            String type = block == null ? null : block.get("type").getString();
        ...
        ONode rebuilt = messageNode.getOrNew("content").asArray();
        rebuilt.clear();
        for (ONode block : kept) {
            rebuilt.add(block);
        }
        if (kept.isEmpty()) {
            // content 不得为空数组。Anthropic 同样拒绝空 text 块，故占位必须是非空文本。
            //
            // 【当前不可达，仍保留为纵深防御】纯思考历史消息在装配循环里已被
            // isSkippableThinkingOnlyMessage 整条跳过，进不到这里（已用出站 JSON 实测）。
            // 但该跳过判定与本方法相互独立，任一侧放宽都会让此分支复活，
            // 届时空 text 块会直接导致 400 —— 占位内容不能是空串。
            rebuilt.addNew().set("type", "text").set("text", THINKING_ONLY_PLACEHOLDER_TEXT);
        }
    }
    ...
    private boolean markMessageCacheBreakpoint(ONode messageNode, CacheControl cacheControl) {
        ...
        if (contentNode.isArray()) {
            ...
            for (int i = contentNode.size() - 1; i >= 0; i--) {
                ...
                // 空 text 块同样会被服务端拒（与下方字符串分支同一条约束）：
                // 两个分支必须同口径，否则数组形态会把断点挂到占位空块上而整条请求被拒。
                if ("text".equals(block.get("type").getString())
                        && Utils.isEmpty(block.get("text").getString())) {
                    continue;
                }

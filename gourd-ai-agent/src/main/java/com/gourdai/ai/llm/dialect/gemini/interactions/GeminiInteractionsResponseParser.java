 ...
 */
package com.gourdai.ai.llm.dialect.gemini.interactions;
...
import com.gourdai.ai.llm.dialect.gemini.interactions.model.InteractionStepType;
...
public class GeminiInteractionsResponseParser {
    ...
    private void emitStepDeltaEvent(ChatStreamContext ctx, ONode data) {
        ...
        if ("thought_signature".equals(deltaType)) {
            String signature = delta.get("signature").getString();
            if (Utils.isNotEmpty(signature)) {
                ctx.emit(ctx.event(com.gourdai.ai.chat.event.ChatEventType.THINKING_SIGNATURE)
                        .rawType("step.delta")
                        .text(signature)
                        .raw(data)
                        .build());

    ...
    public static McpClientProvider fromMcpServer(McpServerParameters serverParameters) throws IOException {
        ...
        if (McpChannel.STDIO.equalsIgnoreCase(type)) {
            ...
            if (serverParameters.getTimeout() != null) {
                // 弱网优化（2026-09-18）：与 McpClientProperties#prepare() 共用同一套换算口径——
                // 建连固定 10s、写/读取配置值但不低于 1s 下限（原 (int) getSeconds() 会把 <1s
                // 配置截断成 0，而 HttpTimeout.of(0) = 永不超时）。
                builder.httpTimeout(McpClientProperties.resolveHttpTimeout(serverParameters.getTimeout()));

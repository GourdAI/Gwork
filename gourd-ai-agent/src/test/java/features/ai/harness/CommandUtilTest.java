...
public class CommandUtilTest {
    ...
    public void testParseArguments() {
        ...
        rst = CmdUtil.parseArguments("/loop cron:\"1 * * * * ? \" 查下淘宝网最新的推荐商品");
        ...
        // 2. 简单空格分隔测试
        rst = CmdUtil.parseArguments("/loop 1m 查下淘宝网最新的推荐商品");
        Assertions.assertEquals(3, rst.size());
        Assertions.assertEquals("1m", rst.get(1));

        // 3. 多重连续空格与首尾空格测试
        rst = CmdUtil.parseArguments("  /cmd   arg1    arg2  ");
        Assertions.assertEquals(3, rst.size());
        ...
        Assertions.assertTrue(CmdUtil.parseArguments(null).isEmpty());

        // 6. 只有命令没有参数
        rst = CmdUtil.parseArguments("/help");
        Assertions.assertEquals(1, rst.size());
        Assertions.assertEquals("/help", rst.get(0));

        // 7. 复杂的 Cron + Key-Value
        rst = CmdUtil.parseArguments("/task --id=101 --cron:\"0 0/1 * * * ?\"");
        Assertions.assertEquals(3, rst.size());

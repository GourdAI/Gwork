...
/**
 * 加载 resedit，先按本文件位置解析，再回落到调用方 cwd。
 *
 * 为什么不能只写 require('resedit')：本脚本被两个模块共用。Electron 侧能从
 * gourd-ai-desktop/node_modules 解到；但 Tauri 侧只在自己模块跑 npm install（CI 就是
 * 这么做的），若仅靠默认解析，就变成「打 Tauri 包先得在 Electron 模块 npm install」
 * —— 正是 sync-backend.js 花力气拆掉的那种隐形跨模块依赖。两处都试，哪边装了用哪边。
 */
function requireResEdit() {
  ...
  // paths 指向调用方 cwd（Tauri 的 beforeBuildCommand cwd 即 gourd-ai-tauri/）
  return require(require.resolve('resedit', { paths: [process.cwd()] }));
}
...
/**
 * 推断品牌 ico 路径。两个桌面壳的图标源位置不同，按「调用方 cwd 优先」探测：
 *   - Tauri：<cwd>/src-tauri/icons/icon.ico（tauri.conf.json 的图标唯一来源）
 *   - Electron：<本模块>/resources/icons/icon.ico（品牌图标唯一来源，见 prepare-resources.js）
 * 两端图标内容本就各自维护，不强行统一；取不到时返回 null（降级为只改版本资源）。
 */
...
// Tauri 在自己模块内写 `build/extraResources/jre` 才能如字面所期。
//

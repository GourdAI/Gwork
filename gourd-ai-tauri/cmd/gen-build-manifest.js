 ...
 * 这是 **Tauri 侧的自有副本**，与 Electron 的
 * `gourd-ai-desktop/cmd/gen-build-manifest.js` 同源。此前 Tauri 直接执行 Electron
 * 模块里那一份（tauri.conf.json 里写的是
 * `node ../gourd-ai-desktop/cmd/gen-build-manifest.js --extra-dir ...`），一旦把 Tauri
 * 拆成独立仓库，这条跨模块路径立刻失效。现各持一份、各自演进：改 Tauri 的指纹清单
 * 请改本文件，不要把 ../gourd-ai-desktop 的引用再请回来。同类拆分见 cmd/brand-jre.js。
 *
 * 用法：作为 CLI 由 Tauri 的 beforeBuildCommand 调用
 *   `node cmd/gen-build-manifest.js [--extra-dir <目录>]`
 * 不传 --extra-dir 时默认写本模块自己的 `build/extraResources`，也就是
 * cmd/sync-backend.js 的产出目录（2026-09-06 起两端各自持有暂存目录，不再共用）。
 *
 * 注意：REPO_ROOT 指向仓库根（../..），buildId 与同源校验的 jar 都取自
 * `gourd-ai-agent/target/`。Tauri 独立成仓后这两个路径需一并调整。
 *
...
function assertJarSameSource(jarPath) {
  ...
  if (packedSha !== builtSha) {
    throw new Error(
      ...
      + '  请先执行 mvn -B -ntp -pl gourd-ai-agent -am package -DskipTests，'
      + '再跑 node cmd/sync-backend.js 把 jar 同步到 build/extraResources。'
    );
  }
  return packedSha;
}
...
if (require.main === module) {
  try {
    // --extra-dir：让调用方指定 extraResources 目录。
    // 相对路径按**进程 cwd** 解析：beforeBuildCommand 的 cwd 是 gourd-ai-tauri/，
    // 写 `--extra-dir build/extraResources` 才能如字面所期；不传则回落到 DEFAULT_EXTRA，
    // 在 Tauri 下两者本就是同一个目录。
